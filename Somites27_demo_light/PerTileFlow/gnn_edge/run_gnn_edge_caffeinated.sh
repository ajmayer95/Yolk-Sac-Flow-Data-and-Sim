...
DEMO_ROOT="$(cd "$PROJECT_ROOT/.." && pwd)"
STAMP="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="${OUT_DIR:-$PROJECT_ROOT/renders/gnn_edge_dc_${STAMP}}"
CONFIG="${CONFIG:-$DEMO_ROOT/emb1/config.json}"
EPOCHS="${EPOCHS:-100}"
SEED="${SEED:-0}"
PYTHON_BIN="${PYTHON_BIN:-python}"
SWEEP="${SWEEP:-1}"
K_VALUES=(${K_VALUES:-0 1 2 3})
HIDDEN_DIM_VALUES=(${HIDDEN_DIM_VALUES:-128})
LAMBDA_DELTA_VALUES=(${LAMBDA_DELTA_VALUES:-1e-4})
SEEDS=(${SEEDS:-$SEED})
...
cmd=(caffeinate -dimsu "$PYTHON_BIN" -X faulthandler gnn_edge/train_gnn_edge.py
  --config "$CONFIG"
  --epochs "$EPOCHS"
  --seed "$SEED"
  --out-dir "$OUT_DIR"
