...
Example:
    python gnn_edge_v2/gnn_edge_dashboard.py \
      --results-dir renders/gnn_edge_dc_20260616_092543
"""
...
<body>
  ...
  <div class="controls">
    ...
    <label><input id="excludeOutlier" type="checkbox" checked> Exclude outlier <code>{html.escape(outlier)}</code></label>
  ...
  <div id="selected"></div>
  <script id="payload" type="application/json">{json.dumps(payload, allow_nan=True)}</script>
  <script>
    ...
    function activeRows() {{
      ...
      if (exclude.checked) rows = rows.filter(r => r.run !== data.outlier);
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ap.add_argument("--outlier", default="K0_hidden32_lambda0p0001_seed0")
    ...
    for mode, active in (
        ("included", rows),
        ("excluded", [r for r in rows if r["run"] != args.outlier]),
    ):
