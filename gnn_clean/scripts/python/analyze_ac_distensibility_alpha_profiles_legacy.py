...
def regime_for_label(label: str) -> str:
    return REGIME_BY_PREFIX.get(label_prefix(label), "")
...
def main() -> None:
    ...
    for summary_path in sorted(input_root.glob("*/alpha_*/D0_*/summary.csv")):
        ...
        for _, series in table.iterrows():
            ...
            row["selection_display_label"] = display_label(label)
    ...
    for (model_name, label, alpha), group in combined.groupby(
        ...
        regime = regime_for_label(str(label))
            ...
            metric_minima.append(
                {
                    ...
                    "selection_display_label": display_label(str(label)),
    ...
    representatives = sorted(
        ...
        key=lambda row: (
            str(row["model_name"]),
            label_sort_key(str(row["representative_label"])),
            float(row["alpha"]),
        ),
    )
