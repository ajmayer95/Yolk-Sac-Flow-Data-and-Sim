...
MODULAR_SRC = Path(__file__).resolve().parent / "src"
sys.path.insert(0, str(PROJECT_ROOT / "scripts"))
sys.path.insert(0, str(PROJECT_ROOT))
sys.path.insert(0, str(MODULAR_SRC))
...
from pertile.gnn_edge.constants import MU, PX_SIZE_M, nL_per_m3  # noqa: E402
...
def _harmonic_lambdas(args, n_harmonics: int, device: torch.device,
                      dtype: torch.dtype) -> torch.Tensor:
    vals = [float(args.lambda_h1), float(args.lambda_h2)]
    return torch.tensor(vals[:n_harmonics], device=device, dtype=dtype)


def _loss_harmonics_nls(q_hat_h: torch.Tensor, q_obs_h: torch.Tensor,
                        valid_h: torch.Tensor, weight_h: torch.Tensor,
                        edge_mask: torch.Tensor, args) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    lam = _harmonic_lambdas(args, q_hat_h.shape[1], q_hat_h.device, q_hat_h.dtype)
...
def train_physics_model(data: MosaicData, train_mask: torch.Tensor, args,
    ...
    model = EdgeCorrectionGNN(
        ...
        hidden_dim=args.hidden_dim, n_layers=args.layers,
        n_harmonics=data.q_harmonic_obs.shape[1]).to(device)
    ...
    for epoch in iterator:
        ...
        harmonic_loss, harmonic_losses = _loss_harmonics_nls(
            q_hat_h, d.q_harmonic_obs, d.harmonic_valid_mask,
            d.harmonic_loss_weight, train_mask, args)
        ...
        row = {
            ...
            "h1_loss": float(harmonic_losses[0].detach().cpu()) if harmonic_losses.numel() > 0 else float("nan"),
            "h2_loss": float(harmonic_losses[1].detach().cpu()) if harmonic_losses.numel() > 1 else float("nan"),
        ...
        if has_validation:
            with torch.no_grad():
                ...
                train_h_loss, _ = _loss_harmonics_nls(
                    eval_q_hat_h, d.q_harmonic_obs, d.harmonic_valid_mask,
                    d.harmonic_loss_weight, train_mask, args)
                val_h_loss, _ = _loss_harmonics_nls(
                    eval_q_hat_h, d.q_harmonic_obs, d.harmonic_valid_mask,
                    d.harmonic_loss_weight, val_mask_dev, args)
...
@torch.no_grad()
def collect_harmonic_rows(data: MosaicData, q_hat_h: torch.Tensor,
...
def parse_args(argv: Optional[Sequence[str]] = None):
    ...
    ap.add_argument("--lambda-h1", type=float, default=1.0,
                    help="weight for the SNR-weighted H1 auxiliary flow loss")
    ap.add_argument("--lambda-h2", type=float, default=1.0,
                    help="weight for the SNR-weighted H2 auxiliary flow loss")
    ...
    ap.add_argument("--flow-components", choices=("dc", "dc-h1", "dc-h1-h2"),
                    default="dc",
