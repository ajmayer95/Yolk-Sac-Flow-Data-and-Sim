"""Create a static dashboard for synthetic velocity-target gnn_edge DC outputs.

Example:
    python gnn_edge_dashboard.py

By default this local copy reads ``gnn_edge_velocity_dc`` in the same folder and
...
DEFAULT_RESULTS_DIR = RESULTS_DIR / "gnn_edge_velocity_dc"
...
NUMERIC_COLUMNS = {
    ...
    "train_RMSE_v_m_s", "val_RMSE_v_m_s",
    "train_RMSE_v_mm_s", "val_RMSE_v_mm_s",
    "train_NRMSE_v", "val_NRMSE_v",
    "train_MAE_v_m_s", "val_MAE_v_m_s",
...
GALLERY_IMAGES = [
    "validation_loss.png",
    "training_loss.png",
    "physics_gnn_v_pred_vs_obs.png",
    "residual_vs_V_obs.png",
    "abs_error_vs_abs_V_obs.png",
    "signed_velocity_error_map.png",
    "absolute_velocity_error_map.png",
    "binned_error_by_abs_V_obs.png",
    "delta_hist.png",
    "C_hist_logx.png",
    "delta_vs_log_radius.png",
    "delta_vs_log_G_pois.png",
    "delta_vs_abs_V_obs.png",
    "delta_vs_velocity_residual.png",
...
def coerce_numeric(row: dict) -> dict:
    ...
    if "out_dir" in row:
        ...
        row["run"] = ""
    row["metric_NRMSE"] = safe_float(row.get("val_NRMSE_v", row.get("val_NRMSE")))
    row["metric_RMSE"] = safe_float(row.get("val_RMSE_v_m_s", row.get("val_RMSE")))
    row["metric_MAE"] = safe_float(row.get("val_MAE_v_m_s", row.get("val_MAE")))
    return row
...
def group_rows(rows: Sequence[dict], factor: str) -> List[dict]:
    ...
    for level in sorted({row[factor] for row in rows}):
        vals = [float(row["metric_NRMSE"]) for row in rows if row[factor] == level]
        ...
        out.append({
            ...
            "level": level,
            "n": len(vals),
            "mean_metric_NRMSE": mean(vals),
            "median_metric_NRMSE": median(vals),
            "min_metric_NRMSE": min(vals),
            "max_metric_NRMSE": max(vals),
        })
    return out


def factor_statistic(rows: Sequence[dict], factor: str):
    y = [float(row["metric_NRMSE"]) for row in rows]
...
def can_compute_factor(rows: Sequence[dict], factor: str) -> bool:
    vals = [safe_float(r.get("metric_NRMSE")) for r in rows]
...
def best_summary(rows: Sequence[dict]) -> dict:
    finite = [r for r in rows if math.isfinite(float(r.get("metric_NRMSE", float("nan"))))]
    if not finite:
        return {}
    best = min(finite, key=lambda r: float(r["metric_NRMSE"]))
    worst = max(finite, key=lambda r: float(r["metric_NRMSE"]))
    return {
        "n": len(finite),
        "mean_metric_NRMSE": mean([r["metric_NRMSE"] for r in finite]),
        "median_metric_NRMSE": median([r["metric_NRMSE"] for r in finite]),
        "best_run": best["run"],
        "best_metric_NRMSE": best["metric_NRMSE"],
        "worst_run": worst["run"],
        "worst_metric_NRMSE": worst["metric_NRMSE"],
    }
...
<head>
  <meta charset="utf-8">
  <title>Synthetic GNN Edge Velocity Dashboard</title>
  <style>
...
<body>
  <h1>Synthetic GNN Edge Velocity Dashboard</h1>
  ...
  <div class="controls">
    ...
    <label>Sort runs by<br>
      <select id="sortKey">
        <option value="metric_NRMSE">velocity NRMSE</option>
        <option value="metric_RMSE">velocity RMSE</option>
  ...
  <script>
    ...
    function renderSelect() {{
      ...
      for (const r of activeRows()) {{
        ...
        opt.value = r.run;
        opt.textContent = `${{r.run}}  velocity_NRMSE=${{fmt(r.metric_NRMSE)}}`;
    ...
    function renderCards() {{
      ...
      const vals = rows.map(r => +r.metric_NRMSE).filter(Number.isFinite);
      const best = rows.reduce((a,b) => (+b.metric_NRMSE < +a.metric_NRMSE ? b : a), rows[0]);
      ...
      document.getElementById('cards').innerHTML = `
        ...
        <div class="card"><b>Best velocity NRMSE</b>${{fmt(best?.metric_NRMSE)}}</div>
        <div class="card"><b>Median velocity NRMSE</b>${{fmt(median)}}</div>
      `;
    }}
    ...
    function renderRuns() {{
      let html = '<table><tr><th>run</th><th>K</th><th>hidden</th><th>lambda</th><th>velocity NRMSE</th><th>velocity RMSE</th><th>val Pearson</th><th>median C</th><th>max C</th></tr>';
      for (const r of activeRows()) {{
        html += `<tr><td style="text-align:left">${{r.run}}</td><td>${{r.K}}</td><td>${{r.hidden_dim}}</td><td>${{r.lambda_delta}}</td><td>${{fmt(r.metric_NRMSE)}}</td><td>${{fmt(r.metric_RMSE)}}</td><td>${{fmt(r.val_Pearson)}}</td><td>${{fmt(r.median_C)}}</td><td>${{fmt(r.max_C)}}</td></tr>`;
      }}
      html += '</table>';
    ...
    function renderSelected() {{
      ...
      let html = `<h3>${{run}}</h3><p class="muted">K=${{row.K}}, hidden=${{row.hidden_dim}}, lambda=${{row.lambda_delta}}, velocity_NRMSE=${{fmt(row.metric_NRMSE)}}, val_Pearson=${{fmt(row.val_Pearson)}}</p>`;
