"""Physics-embedded GNN for whole-mosaic DC edge-velocity prediction.
...
The optimization target is velocity; flow diagnostics are reported in nL/s.
Internal graph solves use SI units and conserve volumetric flow.
"""
...
def first_present(edge_data: dict, names: Sequence[str]):
    for name in names:
...
def edge_q_dc_nls(edge_data: dict, u, v) -> float:
    q = first_present(edge_data, (
        "Q_DC", "mean_Q_piv", "mean_Q", "mean_Q_nL_s",
    ))
    q = safe_float(q)
...
def edge_v_dc_m_s(edge_data: dict, u, v) -> Tuple[float, str]:
    v = first_present(edge_data, (
    ...
    ft = edge_data.get("flow_to")
    if ff is None or ft is None:
        return v, "direct_velocity_metadata"
    return (v if (ff == u and ft == v) else -v), "direct_velocity_metadata"
...
def build_mosaic_data(graph, include_harmonic_features: bool) -> MosaicData:
    ...
    for u, v, ed in graph.edges(data=True):
        ...
        v_m_s, v_source = edge_v_dc_m_s(ed, u, v)
        if not math.isfinite(v_m_s) and math.isfinite(q_nls):
            v_m_s = (q_nls / nL_per_m3) / max(area, 1e-30)
            v_source = "derived_from_flow"
        sign = 1.0
        ff = ed.get("flow_from")
        ft = ed.get("flow_to")
        ...
        v_obs.append(v_m_s if math.isfinite(v_m_s) else 0.0)
        valid_obs.append(math.isfinite(v_m_s))
        target_sources.append(v_source)
    ...
    v_arr = np.asarray(v_obs, dtype=np.float32)
    valid = np.asarray(valid_obs, dtype=bool)
    q_arr = np.where(valid, q_arr, 0.0).astype(np.float32)
    v_arr = np.where(valid, v_arr, 0.0).astype(np.float32)
    ...
    valid_sources = {s for s, ok in zip(target_sources, valid_obs) if ok}
    if valid_sources == {"direct_velocity_metadata"}:
        target_source = "direct_velocity_metadata"
    elif valid_sources == {"derived_from_flow"}:
        target_source = "derived_from_flow"
    elif valid_sources:
        target_source = "mixed_velocity_metadata_and_flow"
    else:
        target_source = "missing"
    print(f"Velocity target source: {target_source}")
...
class DirectFlowGNN(nn.Module):
    ...
    def forward(self, data: MosaicData) -> torch.Tensor:
        ...
        for layer in self.layers:
            h, e = layer(h, e, data.edge_index)
        return self.v_head(e).squeeze(-1)
...
def solve_pressures(data: MosaicData, g_hat: torch.Tensor,
    ...
    L_red = L_red + torch.eye(L_red.shape[0], device=solve_device, dtype=g_solve.dtype) * float(jitter)
...
def physics_forward(data: MosaicData, delta: torch.Tensor,
                    jitter: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    g_hat = data.g_pois.to(delta.device) * torch.exp(delta)
    p = solve_pressures(data, g_hat, jitter=jitter)
    src, dst = data.edge_index
    q_hat = g_hat * (p[src] - p[dst])
    v_hat = q_hat / data.area_m2.to(device=q_hat.device, dtype=q_hat.dtype).clamp_min(1e-30)
    return q_hat, v_hat, p, g_hat


def poisson_baseline(data: MosaicData, jitter: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
...
def _loss_mse_velocity(v_hat: torch.Tensor, v_obs: torch.Tensor,
                       mask: torch.Tensor) -> torch.Tensor:
    if not bool(mask.any()):
        return v_hat.new_tensor(0.0)
    resid = v_hat - v_obs
    return (resid[mask] ** 2).mean()


def _rmse_nrmse_velocity(v_hat: torch.Tensor, v_obs: torch.Tensor,
    ...
    obs_rms = torch.sqrt((obs[mask] ** 2).mean()).clamp_min(1e-30)
...
def train_physics_model(data: MosaicData, train_mask: torch.Tensor, args,
    ...
    for epoch in iterator:
        opt.zero_grad()
        delta = model(d)
        _q_hat, v_hat, _, _ = physics_forward(d, delta, args.jitter)
        v_loss = _loss_mse_velocity(v_hat, d.v_obs, train_mask)
        delta_loss = (delta ** 2).mean()
        loss = v_loss + float(args.lambda_delta) * delta_loss
        loss.backward()
        ...
        if has_validation:
            with torch.no_grad():
                eval_delta = model(d)
                _eval_q_hat, eval_v_hat, _, _ = physics_forward(d, eval_delta, args.jitter)
                train_loss = _loss_mse_velocity(eval_v_hat, d.v_obs, train_mask)
                val_loss = _loss_mse_velocity(eval_v_hat, d.v_obs, val_mask_dev)
                train_rmse, train_nrmse = _rmse_nrmse_velocity(eval_v_hat, d.v_obs, train_mask)
                val_rmse, val_nrmse = _rmse_nrmse_velocity(eval_v_hat, d.v_obs, val_mask_dev)
        ...
        if hasattr(iterator, "set_postfix"):
            postfix = {"loss": f"{row['loss']:.3e}", "v": f"{row['v_loss']:.3e}"}
...
def train_direct_model(data: MosaicData, train_mask: torch.Tensor, args,
    ...
    for epoch in iterator:
        opt.zero_grad()
        v_hat = model(d)
        loss = _loss_mse_velocity(v_hat, d.v_obs, train_mask)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
        opt.step()
        row = {"epoch": epoch, "model": "direct_gnn", "loss": float(loss.detach().cpu()), "v_loss": float(loss.detach().cpu()), "q_loss": float("nan"), "delta_loss": float("nan")}
        history.append(row)
        if hasattr(iterator, "set_postfix"):
...
@torch.no_grad()
def evaluate_arrays(data: MosaicData, v_hat: torch.Tensor,
                    mask: torch.Tensor) -> dict:
    v = data.v_obs.detach().cpu().numpy()
    pred = v_hat.detach().cpu().numpy()
    m = mask.detach().cpu().numpy().astype(bool)
    if not np.any(m):
        return {"n": 0, "RMSE_v_m_s": np.nan, "RMSE_v_mm_s": np.nan, "normalized_RMSE_v": np.nan, "MAE_v_m_s": np.nan, "MAE_v_mm_s": np.nan, "pearson_corr": np.nan, "R2": np.nan}
    resid = pred[m] - v[m]
    rmse = float(np.sqrt(np.mean(resid ** 2)))
    mae = float(np.mean(np.abs(resid)))
    obs_rms = float(np.sqrt(np.mean(v[m] ** 2)))
    if np.std(v[m]) <= 1e-30 or np.std(pred[m]) <= 1e-30:
        corr = np.nan
    else:
        corr = float(np.corrcoef(v[m], pred[m])[0, 1])
    ss_res = float(np.sum((v[m] - pred[m]) ** 2))
    ss_tot = float(np.sum((v[m] - np.mean(v[m])) ** 2))
    return {
        "n": int(np.sum(m)),
        "RMSE_v_m_s": rmse,
        "RMSE_v_mm_s": rmse * 1e3,
        "normalized_RMSE_v": float(rmse / max(obs_rms, 1e-30)),
        "MAE_v_m_s": mae,
        "MAE_v_mm_s": mae * 1e3,
        "pearson_corr": corr,
...
def collect_edge_rows(data: MosaicData, q_hat: torch.Tensor, p: torch.Tensor,
    ...
    v_obs = data.v_obs.cpu().numpy()
    area = data.area_m2.cpu().numpy()
    v_pred = q_pred / nL_per_m3 / np.maximum(area, 1e-30)
    ...
    for ei, (u, v) in enumerate(data.edge_ids):
        ...
        rows.append({
            ...
            "V_obs_m_s": float(v_obs[ei]),
            "V_hat_m_s": float(v_pred[ei]),
            "residual_v_m_s": float(v_obs[ei] - v_pred[ei]),
            "abs_error_v_m_s": float(abs(v_obs[ei] - v_pred[ei])),
...
def enrich_edge_rows(data: MosaicData, graph, edge_rows: Sequence[dict]) -> List[dict]:
    ...
    for row in edge_rows:
        ...
        new.update({
            ...
            "abs_V_obs_m_s": abs(safe_float(new.get("V_obs_m_s"))),
            "abs_error_v_m_s": abs(safe_float(new.get("residual_v_m_s"))),
...
def collect_regression_edges(data: MosaicData,
    ...
    for row in edge_rows:
        ...
        q_obs = safe_float(row.get("Q_obs_nL_s"), 0.0)
        v_obs = float(row["V_obs_m_s"])
        if not all(math.isfinite(x) for x in (delta, c_val, radius, length, g_pois, v_obs)):
            continue
        ei = int(row["edge_id"])
        rows.append({
            "edge_id": ei,
...
def run_delta_regressions(edge_rows: Sequence[dict], out_dir: Path) -> Tuple[List[dict], List[dict]]:
    ...
    specs = [
        ...
        ("Model B", ["log_G_pois"]),
        ("Model C", ["log_radius", "log_length", "abs_V_obs_m_s",
                     "degree_src", "degree_dst"]),
        ("Model D", ["log_radius", "log_length", "abs_V_obs_m_s",
                     "degree_src", "degree_dst", "distance_to_A", "distance_to_V"]),
        ("Model E", ["log_radius", "log_length", "abs_V_obs_m_s",
...
def plot_prediction(edge_rows: Sequence[dict], out_dir: Path, prefix: str) -> None:
    ...
    v_obs = np.asarray([r["V_obs_m_s"] for r in rows], dtype=float)
    v_hat = np.asarray([r["V_hat_m_s"] for r in rows], dtype=float)
    resid = v_obs - v_hat
    ...
    plt.figure(figsize=(5, 5))
    plt.scatter(v_obs * 1e3, v_hat * 1e3, s=12, alpha=0.6)
    lo = float(np.nanmin([np.nanmin(v_obs * 1e3), np.nanmin(v_hat * 1e3)]))
    hi = float(np.nanmax([np.nanmax(v_obs * 1e3), np.nanmax(v_hat * 1e3)]))
    plt.plot([lo, hi], [lo, hi], color="black", lw=1)
    plt.xlabel("observed velocity (mm/s)")
    plt.ylabel("predicted velocity (mm/s)")
    plt.tight_layout()
    plt.savefig(out_dir / f"{prefix}_v_pred_vs_obs.png", dpi=180)
    plt.close()

    plt.figure(figsize=(6, 4))
    plt.hist(resid * 1e3, bins=50)
    plt.xlabel("residual velocity (mm/s)")
    plt.ylabel("count")
    plt.tight_layout()
...
def _binned_error(rows: Sequence[dict], by_key: str, out_dir: Path,
                  filename: str, xlabel: str,
                  error_key: str = "abs_error_v_m_s",
                  ylabel: str = "mean |V_obs - V_hat| (m/s)") -> None:
    vals = np.asarray([safe_float(r.get(by_key)) for r in rows], dtype=float)
    err = np.asarray([safe_float(r.get(error_key)) for r in rows], dtype=float)
...
def plot_diagnostic_suite(rows: Sequence[dict], node_rows: Sequence[dict],
    ...
    if not valid:
        return
    _scatter(valid, "V_obs_m_s", "residual_v_m_s", out_dir,
             "residual_vs_V_obs.png", "V_obs (m/s)", "V_obs - V_hat (m/s)")
    _scatter(valid, "abs_V_obs_m_s", "abs_error_v_m_s", out_dir,
             "abs_error_vs_abs_V_obs.png", "|V_obs| (m/s)", "|velocity error| (m/s)")
    _binned_error(valid, "abs_V_obs_m_s", out_dir, "binned_error_by_abs_V_obs.png", "|V_obs| (m/s)")
    ...
    _plot_edge_map(valid, "residual_v_m_s", out_dir, "signed_velocity_error_map.png", "V_obs - V_hat (m/s)")
    _plot_edge_map(valid, "abs_error_v_m_s", out_dir, "absolute_velocity_error_map.png", "|velocity error| (m/s)")
    ...
    _scatter(valid, "abs_V_obs_m_s", "delta", out_dir, "delta_vs_abs_V_obs.png", "|V_obs| (m/s)", "delta")
    _scatter(valid, "pressure_drop_Pa", "delta", out_dir, "delta_vs_pressure_drop.png", "pressure drop (Pa)", "delta")
    _scatter(valid, "residual_v_m_s", "delta", out_dir, "delta_vs_velocity_residual.png", "V_obs - V_hat (m/s)", "delta")
    ...
    _scatter(valid, "distance_to_A", "residual_v_m_s", out_dir, "velocity_residual_vs_distance_to_A.png", "distance to A/source", "velocity residual (m/s)")
    _scatter(valid, "distance_to_V", "residual_v_m_s", out_dir, "velocity_residual_vs_distance_to_V.png", "distance to V/sink", "velocity residual (m/s)")
    plot_group_box(valid, "delta", "topology_class", out_dir, "delta_by_topology_class.png", "delta")
    plot_group_box(valid, "residual_v_m_s", "topology_class", out_dir, "velocity_residual_by_topology_class.png", "velocity residual (m/s)")
    plot_group_box(valid, "C", "topology_class", out_dir, "C_by_topology_class.png", "C")
    plot_group_box([r for r in valid if r.get("split") == "val"], "abs_error_v_m_s", "topology_class",
                   out_dir, "validation_velocity_error_by_topology_class.png", "|validation velocity error| (m/s)")
    ...
    _scatter(valid, "harmonic_ratio", "residual_v_m_s", out_dir,
             "velocity_residual_vs_harmonic_ratio.png", "harmonic ratio", "velocity residual (m/s)")
...
def write_top_edge_tables(rows: Sequence[dict], out_dir: Path) -> None:
    def keep(row: dict) -> dict:
        keys = [
            ...
            "residual_nL_s", "V_obs_m_s", "V_hat_m_s", "residual_v_m_s",
            "abs_error_v_m_s", "pressure_drop_Pa", "degree_src", "degree_dst",
        ]
    ...
    specs = [
        ...
        ("top50_largest_abs_V_obs.csv", lambda r: abs(safe_float(r.get("V_obs_m_s"))), True),
        ("top50_largest_abs_velocity_residual.csv", lambda r: abs(safe_float(r.get("residual_v_m_s"))), True),
    ]
...
def metrics_rows(data: MosaicData, outputs: Dict[str, Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]],
                 train_mask: torch.Tensor, val_mask: torch.Tensor) -> List[dict]:
    rows = []
    for name, (q_hat, v_hat, _p, _g) in outputs.items():
        for split, mask in (("train", train_mask), ("val", val_mask), ("all", data.valid_mask)):
            ...
            row.update(evaluate_arrays(data, v_hat, mask))
...
def run_experiment(base_data: MosaicData, args, device: torch.device,
    ...
    with torch.no_grad():
        ...
        if direct_model is not None:
            direct_v = direct_model(d)
            direct_q = direct_v * d.area_m2.to(device=direct_v.device, dtype=direct_v.dtype)
            zero_p = torch.full((d.x_node.shape[0],), float("nan"), device=device)
            direct_out = (direct_q, direct_v, zero_p, torch.full_like(d.g_pois, float("nan")))
    ...
    q_hat, v_hat, p, g_hat = physics_out
    ...
    q_pois, _v_pois, p_pois, g_pois = pois_out
    write_csv(
    ...
    train_metrics = evaluate_arrays(data, v_hat, train_mask)
    val_metrics = evaluate_arrays(data, v_hat, val_mask) if bool(val_mask.any()) else {
        "RMSE_v_m_s": float("nan"), "RMSE_v_mm_s": float("nan"),
        "normalized_RMSE_v": float("nan"),
        "MAE_v_m_s": float("nan"), "MAE_v_mm_s": float("nan"),
    ...
    summary = {
        ...
        "train_loss_final": float(validation_history[-1]["train_loss"]) if validation_history else float(physics_history[-1]["v_loss"]),
        "val_loss_final": float(validation_history[-1]["val_loss"]) if validation_history else float("nan"),
        "train_RMSE_v_m_s": float(train_metrics["RMSE_v_m_s"]),
        "val_RMSE_v_m_s": float(val_metrics["RMSE_v_m_s"]),
        "train_RMSE_v_mm_s": float(train_metrics["RMSE_v_mm_s"]),
        "val_RMSE_v_mm_s": float(val_metrics["RMSE_v_mm_s"]),
        "train_NRMSE_v": float(train_metrics["normalized_RMSE_v"]),
        "val_NRMSE_v": float(val_metrics["normalized_RMSE_v"]),
        "train_MAE_v_m_s": float(train_metrics["MAE_v_m_s"]),
        "val_MAE_v_m_s": float(val_metrics["MAE_v_m_s"]),
...
def plot_sweep_summary(rows: Sequence[dict], out_dir: Path) -> None:
    ...
    for key, filename, xlabel, logx in (
        ...
        ys = np.asarray([safe_float(r.get("val_NRMSE_v", r.get("val_NRMSE"))) for r in rows], dtype=float)
        ...
        if logx:
            plt.xscale("log")
        plt.xlabel(xlabel)
        plt.ylabel("final validation velocity NRMSE")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / filename, dpi=180)
        plt.close()
...
def parse_args(argv: Optional[Sequence[str]] = None):
    ...
    ap.add_argument("--config", default=str(PROJECT_ROOT.parent / "emb1" / "config.json"))
    ap.add_argument("--graph", default=None,
                    help="Graph pickle to train on. Defaults to --synthetic-input when it exists.")
    ap.add_argument("--synthetic-input",
                    default=str(RESULTS_DIR / "synthetic_mosaic_graph.gpickle"),
                    help="Synthetic graph written by default_mosaic_tile_profiles.py.")
    ap.add_argument("--out-dir", default=str(RESULTS_DIR / "gnn_edge_velocity_dc"))
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    synthetic_input = Path(args.synthetic_input).expanduser()
    if args.graph is None and synthetic_input.exists():
        args.graph = str(synthetic_input)
    elif args.graph is None:
        print(
            "Synthetic input not found; falling back to --config mosaic_graph. "
            f"Expected {synthetic_input}"
        )
    set_seed(args.seed)
    try:
    ...
    print(f"Valid velocity edge observations: {int(data.valid_mask.sum())}")
    print(f"Target variable: velocity ({data.target_source})")
