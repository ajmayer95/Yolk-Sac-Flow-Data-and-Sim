...
from tile_mosaic_simulation import (
    ...
    result_edge_harmonics,
)
from synthetic_validation_neumann_bc import F0_HZ, PX_SIZE_M, nL_per_m3
...
def _synthetic_flow_attrs(edge_data: dict, coeffs: np.ndarray, u, v,
                          n_harmonics: int) -> dict:
    """Convert simulated edge harmonics into graph fields read by the GNN.

    ``train_gnn_edge.py`` expects Q magnitudes in nL/s plus a
    ``flow_from``/``flow_to`` orientation.  The solve returns signed complex
    harmonics in the graph edge orientation, so we canonicalize each edge to
    positive DC flow and rotate harmonic phases into that same direction.
    """
    r_m, _l_m = _edge_geometry(edge_data, PX_SIZE_M)
    if r_m is None:
        raise ValueError("missing edge radius for synthetic velocity output")
    r_m = max(float(r_m), 1e-12)
    area_m2 = math.pi * r_m ** 2
    q_dc_signed = float(np.real(coeffs[0])) if len(coeffs) else 0.0
    v_dc_signed = (q_dc_signed / nL_per_m3) / area_m2
    if q_dc_signed >= 0.0:
        flow_from, flow_to = u, v
        orient = 1.0
    else:
        flow_from, flow_to = v, u
        orient = -1.0
    attrs = {
        "Q_DC": abs(q_dc_signed),
        "mean_Q": abs(q_dc_signed),
        "mean_Q_nL_s": abs(q_dc_signed),
        "V_DC_m_s": abs(v_dc_signed),
        "mean_v_m_s": abs(v_dc_signed),
        "mean_velocity_m_s": abs(v_dc_signed),
        "synthetic_V_DC_signed_m_s": v_dc_signed,
        "flow_from": flow_from,
        "flow_to": flow_to,
        "synthetic_Q_DC_signed_nL_s": q_dc_signed,
    }
    for h in range(1, int(n_harmonics) + 1):
        qh = orient * (complex(coeffs[h]) if h < len(coeffs) else 0.0j)
        vh = (qh / nL_per_m3) / area_m2
        attrs[f"Q_H{h}_amp"] = float(abs(qh))
        attrs[f"Q_H{h}_phi"] = float(np.angle(qh))
        attrs[f"Q_H{h}_real_nL_s"] = float(qh.real)
        attrs[f"Q_H{h}_imag_nL_s"] = float(qh.imag)
        attrs[f"Q_H{h}_snr_db"] = 60.0
        attrs[f"V_H{h}_amp_m_s"] = float(abs(vh))
        attrs[f"V_H{h}_phi"] = float(np.angle(vh))
        attrs[f"V_H{h}_real_m_s"] = float(vh.real)
        attrs[f"V_H{h}_imag_m_s"] = float(vh.imag)
        attrs[f"V_H{h}_snr_db"] = 60.0
        if h == 1:
            attrs["amp_Q_h1_piv"] = float(abs(qh))
            attrs["phase_h1_piv"] = float(np.angle(qh))
            attrs["amp_Q"] = float(abs(qh))
            attrs["phase"] = float(np.angle(qh))
        elif h == 2:
            attrs["amp_Q_h2_piv"] = float(abs(qh))
            attrs["phase_h2_piv"] = float(np.angle(qh))
    return attrs


def _write_synthetic_graph_outputs(graph, mosaic_result, out_dir: Path,
                                   args, graph_path: Path) -> Path:
    synth_graph = copy.deepcopy(graph)
    rows = []
    n_written = 0
    for u, v in synth_graph.edges():
        coeffs = result_edge_harmonics(mosaic_result, u, v)
        if coeffs is None or len(coeffs) == 0:
            continue
        attrs = _synthetic_flow_attrs(
            synth_graph.edges[u, v],
            np.asarray(coeffs, dtype=complex), u, v,
            int(args.n_harmonics_mosaic))
        synth_graph.edges[u, v].update(attrs)
        row = {
            "u": u,
            "v": v,
            "flow_from": attrs["flow_from"],
            "flow_to": attrs["flow_to"],
            "Q_DC_nL_s": attrs["Q_DC"],
            "Q_DC_signed_u_to_v_nL_s": attrs["synthetic_Q_DC_signed_nL_s"],
            "V_DC_m_s": attrs["V_DC_m_s"],
            "V_DC_signed_u_to_v_m_s": attrs["synthetic_V_DC_signed_m_s"],
        }
        for h in range(1, int(args.n_harmonics_mosaic) + 1):
            row[f"Q_H{h}_amp_nL_s"] = attrs[f"Q_H{h}_amp"]
            row[f"Q_H{h}_phi_rad"] = attrs[f"Q_H{h}_phi"]
            row[f"Q_H{h}_real_nL_s"] = attrs[f"Q_H{h}_real_nL_s"]
            row[f"Q_H{h}_imag_nL_s"] = attrs[f"Q_H{h}_imag_nL_s"]
            row[f"V_H{h}_amp_m_s"] = attrs[f"V_H{h}_amp_m_s"]
            row[f"V_H{h}_phi_rad"] = attrs[f"V_H{h}_phi"]
            row[f"V_H{h}_real_m_s"] = attrs[f"V_H{h}_real_m_s"]
            row[f"V_H{h}_imag_m_s"] = attrs[f"V_H{h}_imag_m_s"]
        rows.append(row)
        n_written += 1

    synth_graph.graph["synthetic_mosaic_flow"] = {
        "source_graph": str(graph_path),
        "D_mosaic": float(args.D_mosaic),
    ...
    graph_out = out_dir / _out_name("synthetic_mosaic_graph.gpickle", args)
    with open(graph_out, "wb") as f:
        pickle.dump(synth_graph, f, protocol=pickle.HIGHEST_PROTOCOL)
    _write_csv(out_dir / _out_name("synthetic_mosaic_edge_flows.csv", args),
               rows)
    meta = {
        "synthetic_graph": str(graph_out),
        "source_graph": str(graph_path),
        "n_edges_with_synthetic_flows": int(n_written),
        "D_mosaic": float(args.D_mosaic),
        "target_flux_nL_s": float(args.target_flux),
        "n_harmonics_mosaic": int(args.n_harmonics_mosaic),
        "f0_hz": float(args.f0_hz),
        "train_command": (
            "python train_gnn_edge.py "
            f"--graph {graph_out.name} --config {args.config}"
        ),
    }
    (out_dir / _out_name("synthetic_mosaic_metadata.json", args)).write_text(
        json.dumps(_clean_json_value(meta), indent=2, allow_nan=False),
        encoding="utf-8")
...
def main() -> None:
    ...
    synthetic_graph_path = _write_synthetic_graph_outputs(
        graph, mosaic_result, out_dir, args, graph_path)
    ...
    payload = {
        "defaults": {
            ...
            "synthetic_graph": str(synthetic_graph_path),
        },
