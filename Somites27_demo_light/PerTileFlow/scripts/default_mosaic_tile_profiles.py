...
Default assumptions:
  ...
  * measured boundary waveforms from graph node ``bc_harmonics`` when present
  * explicit source/sink nodes can be supplied for Somites27 graphs without
    stored boundary annotations
...
def _run_default_mosaic_solve(graph, D: float, f0_hz: float,
    ...
    if source_nodes or sink_nodes:
        if not source_nodes or not sink_nodes:
            raise SystemExit("Provide both --source-nodes and --sink-nodes.")
        boundary_nodes = [int(n) for n in list(source_nodes) + list(sink_nodes)]
        missing = [n for n in boundary_nodes if n not in graph]
        if missing:
            raise SystemExit(f"Boundary node IDs not in graph: {missing}")
        for n in source_nodes:
            graph.nodes[int(n)]["boundary_type"] = "source"
        for n in sink_nodes:
            graph.nodes[int(n)]["boundary_type"] = "sink"
        bc_override = None
    else:
        boundary_nodes = [n for n, d in graph.nodes(data=True)
                          if d.get("boundary_type") is not None]
        if not boundary_nodes:
            raise SystemExit(
                "No graph boundary annotations found. Somites27 currently "
                "has no node boundary_type/bc_harmonics metadata. Provide "
                "--source-nodes and --sink-nodes to extract measured "
                "boundary harmonics from incident edge Q_t, or annotate the "
                "graph with boundary_type and bc_harmonics.")
    ...
    return solve_transmission_line(
        ...
        f0_hz=float(f0_hz), boundary_nodes=boundary_nodes,
...
def main() -> None:
    ...
    ap.add_argument("--source-nodes", nargs="*", type=int, default=None,
                    help="Explicit source boundary node IDs for datasets "
                         "without boundary_type/bc_harmonics metadata.")
    ap.add_argument("--sink-nodes", nargs="*", type=int, default=None,
                    help="Explicit sink boundary node IDs for datasets "
                         "without boundary_type/bc_harmonics metadata.")
    ...
    print(f"D grid: {len(D_grid)} values from {D_grid[0]:.3e} "
          f"to {D_grid[-1]:.3e}", flush=True)
    print(f"Output: {out_dir}", flush=True)
    boundary_mode = ("explicit measured source/sink nodes"
                     if (args.source_nodes or args.sink_nodes)
                     else "graph boundary annotations")
    print("Viewer-default simulation config:", flush=True)
    print(f"  D={args.D_mosaic:.3e}, target_flux={args.target_flux:.3g} nL/s, "
          f"n_harmonics={args.n_harmonics_mosaic}, "
          f"boundary_mode={boundary_mode}", flush=True)
    objective_label = _objective_name(args).replace("_", " ")
    print(f"Tile profile objective: {objective_label}", flush=True)

    t0 = time.time()
    if args.reuse_mosaic_result:
        print("Loading reused mosaic solve...", flush=True)
        ...
        t_solve = time.time()
        print("Running whole-mosaic transmission-line solve...", flush=True)
        mosaic_result = _run_default_mosaic_solve(
            ...
            verbose=True,
            source_nodes=args.source_nodes or (),
            sink_nodes=args.sink_nodes or ())
        print(f"Whole-mosaic solve finished in "
              f"{_elapsed(time.time() - t_solve)}", flush=True)
    ...
    print("Running tile profile scans...", flush=True)
    ...
    for i, tile_id in enumerate(tiles, start=1):
        ...
        eta = ""
        if done:
            per_tile = (time.time() - t_profiles) / done
            eta = f", ETA {_elapsed(per_tile * (len(tiles) - done))}"
        print(f"[{i}/{len(tiles)}] tile {tile_id} "
              f"({len(D_grid)} D values{eta})", flush=True)
        try:
        ...
        print(f"  D_hat={metrics['D_hat']:.3e}  "
              ...
              f"elapsed={_elapsed(time.time() - t_tile)}", flush=True)

    print(f"Tile profile scans finished in "
          f"{_elapsed(time.time() - t_profiles)}", flush=True)
    print("Summing global/interior/periphery profiles...", flush=True)
    ...
    csv_paths = [
        out_dir / _out_name("global_profile_constant_D.csv", args),
        out_dir / _out_name("interior_profile_constant_D.csv", args),
        out_dir / _out_name("periphery_profile_constant_D.csv", args),
        out_dir / _out_name("tile_profiles.csv", args),
        out_dir / _out_name("tile_profile_summary.csv", args),
    ]
    ...
    print("Building dashboard payload...", flush=True)
    tile_payload = {}
    for tile_id, profile in tile_profiles.items():
        chi_min = min(p["chi2"] for p in profile)
    ...
    print(f"Done in {_elapsed(time.time() - t0)}", flush=True)
