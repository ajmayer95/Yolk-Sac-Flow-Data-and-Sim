...
def apply_metric_y_axis(ax: plt.Axes, values: np.ndarray) -> bool:
    ...
    if not math.isfinite(span) or span <= 0.0:
        span = max(abs(lower_data) * 0.05, 1.0e-6)
    if lower_data >= 0.0 and lower_data < 0.05:
        ax.set_ylim(0.0, upper_data + 0.08 * span)
        ax.yaxis.set_minor_locator(AutoMinorLocator(2))
        return False
    apply_truncated_linear_axis(ax, "y", values, pad_fraction=0.42, upper_pad_fraction=0.06, nbins=5)
    return True
...
def plot_metric_panel(
    ...
    needs_break_y = apply_metric_y_axis(ax, metric_values)
    ax.set_xscale("log")
    ...
    if title is not None:
        ax.set_title(title)
    add_axis_break_marks(ax, break_y=needs_break_y)
...
def plot_phase_panel(
    ...
    if finite_phase.size:
        lower = min(0.0, float(np.min(finite_phase)))
        upper = float(np.max(finite_phase))
        span = max(upper - lower, 1.0e-6)
        ax.set_ylim(lower - 0.04 * span, upper + 0.06 * span)
        ax.yaxis.set_minor_locator(AutoMinorLocator(2))
    ax.set_xscale("log")
    ax.set_xlabel(r"$D_0\ [\mathrm{Pa}^{-1}]$")
    if ylabel is not None:
...
def plot_pareto_panel(
    ...
    apply_truncated_linear_axis(ax, "x", finite_x, pad_fraction=0.22, upper_pad_fraction=0.06, nbins=5)
    apply_truncated_linear_axis(ax, "y", finite_y, pad_fraction=0.22, upper_pad_fraction=0.06, nbins=5)
    ...
    if title is not None:
        ax.set_title(title)
    add_axis_break_marks(ax, break_x=True, break_y=True)
...
def plot_model_grid(
    ...
    fig, axes = plt.subplots(3, 4, figsize=(18.0, 11.2), constrained_layout=True)
    column_titles = (
        "Complex Flow RMSE",
        "Kirchhoff RMS",
    ...
    for row_idx, label in enumerate(LABEL_ORDER):
        ...
        display_label = DISPLAY_LABEL_BY_CODE[label]
        ...
        plot_metric_panel(
            ...
            ylabel="Kirchhoff RMS [nL/s]" if row_idx > 0 else f"{display_label}\nKirchhoff RMS [nL/s]",
            title=column_titles[1] if row_idx == 0 else None,
        )
        plot_phase_panel(
            axes[row_idx, 2],
            label_df,
            ylabel="Phase Difference [deg]" if row_idx > 0 else f"{display_label}\nPhase Difference [deg]",
        ...
        plot_pareto_panel(
            ...
            label_df,
            label_rep_df,
            ylabel="Complex Flow RMSE [nL/s]" if row_idx > 0 else f"{display_label}\nComplex Flow RMSE [nL/s]",
