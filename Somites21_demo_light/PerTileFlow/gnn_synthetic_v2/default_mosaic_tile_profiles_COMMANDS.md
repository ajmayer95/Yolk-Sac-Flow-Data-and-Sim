# `gnn_synthetic_v2` Velocity Workflow Command Reference

This v2 workflow keeps the physical pressure solve in volumetric flow, but
uses velocity as the observed/training quantity:

```text
G_e = pi r_e^4 / (8 mu L_e)
Q_hat_e = G_hat_e (P_i - P_j)
v_hat_e = Q_hat_e / (pi r_e^2)
```

The nodal solve still conserves `Q`; it is not assembled with velocity
conductance. The GNN data loss compares `v_hat` to `v_obs`, and no extra mass
loss is needed because conservation is enforced by the pressure solve. `Q_obs`
and `Q_hat` remain in CSVs and plots as diagnostics.

Run the whole known-distensibility pipeline from this folder:
...
Each value writes to a folder such as:
...
Inside each result folder, the trained velocity-target GNN output is written to
`gnn_edge_velocity_dc/`, and the tile distensibility profile inference uses
`--observation-variable velocity` by default.

Run from `Somites21_demo/PerTileFlow`:

```bash
python gnn_synthetic_v2/default_mosaic_tile_profiles.py --config ../emb1/config.json
```
...
```bash
python gnn_synthetic_v2/default_mosaic_tile_profiles.py \
  --config ../emb1/config.json
```
...
```bash
python gnn_synthetic_v2/infer_default_mosaic_tile_profiles.py \
  --config ../emb1/config.json \
  --observation-variable velocity
```
...
python scripts/default_mosaic_tile_profiles.py \
  --config ../emb1/config.json \
  --out-dir gnn_synthetic_v2/d0p001_results
```
