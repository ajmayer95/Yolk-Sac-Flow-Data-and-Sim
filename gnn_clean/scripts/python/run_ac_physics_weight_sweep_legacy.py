...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Optional override applied to every harmonic in the AC Step 2 sweep.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    for harmonic_number in harmonic_numbers:
        ...
        lambda_b = (
            float(args.lambda_b_override)
            if args.lambda_b_override is not None
            else DEFAULT_LAMBDA_B_BY_HARMONIC[harmonic_number]
        )
