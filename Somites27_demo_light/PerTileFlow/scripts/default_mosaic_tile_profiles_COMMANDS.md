...
Run from `Somites27_demo/PerTileFlow`:
...
Run the simulation/recovery profile workflow. Somites27 needs explicit
source/sink nodes for the whole-mosaic solve:

```bash
python scripts/default_mosaic_tile_profiles.py \
  --config ../emb1/config.json \
  --source-nodes 9811 87672 \
  --sink-nodes 8050 87666
```
...
For the two measured-data inference scripts, `--source-nodes` and
`--sink-nodes` are recorded for provenance only. They are not used in the fit
because those scripts do not run a whole-mosaic solve.
...
2. Runs one whole-mosaic transmission-line solve. If the graph has stored
   `boundary_type`/`bc_harmonics`, those are used. Somites27 currently lacks
   those annotations, so provide `--source-nodes` and `--sink-nodes` to extract
   measured boundary harmonics from incident edge `Q_t`.
...
Explicit source boundary node IDs for Somites27 graphs without stored
`boundary_type`/`bc_harmonics` metadata.

Use this together with `--sink-nodes`. The script temporarily labels these
nodes as source boundaries and lets the transmission-line solver extract
boundary harmonics from incident edge `Q_t`.

### `--sink-nodes NODE ...`

Explicit sink boundary node IDs for Somites27 graphs without stored
`boundary_type`/`bc_harmonics` metadata.
...
python scripts/default_mosaic_tile_profiles.py \
  --config ../emb1/config.json \
  --source-nodes SOURCE_ID_1 SOURCE_ID_2 \
  --sink-nodes SINK_ID_1 SINK_ID_2
```

If these are omitted and the graph has no boundary annotations, the script
will stop before the mosaic solve because the whole-mosaic boundary-value
...
boundary-condition setup. For explicit Somites27 `--source-nodes` /
`--sink-nodes`, measured boundary harmonics are extracted from incident edge
`Q_t`, so this value is metadata rather than the source of the harmonic
waveform.
...
For Somites27, `--source-nodes` and `--sink-nodes` are accepted and written
to the output metadata, but they are not used in the fit.
...
```text
1, 2, 3, 4, 5, 6, 7, 8, 14, 15, 23, 24, 32, 33,
41, 42, 51, 52, 60, 61, 68, 69, 75, 76, 81, 83,
84, 86, 87
```
